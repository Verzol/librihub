...
export default function AvailableDeliveriesPage() {
  ...
  return (
    <>
      <PageHeader title="Nhiệm vụ giao sách khả dụng" description="Chỉ courier đã được duyệt và đang sẵn sàng mới thấy danh sách này." />
      {error ? <Alert variant="error">{error}</Alert> : null}
      {tasks.length === 0 ? <EmptyState title="Không có nhiệm vụ nào" /> : null}
      <div className="flex flex-col gap-4">
        {tasks.map((task) => (
          <Card key={task.transaction_id}>
            <h2 className="text-sm font-semibold text-slate-900">Giao dịch #{task.transaction_id}</h2>
            <p className="mt-1 text-sm text-slate-500">
              Sách #{task.book_id} · chủ sách #{task.owner_id} · người yêu cầu #{task.requester_id} · {formatDate(task.requested_at)}
            </p>
            <p className="mt-2 text-sm text-slate-500">Lấy: {task.pickup_address ?? "Chưa có điểm lấy"}</p>
            <p className="text-sm text-slate-500">Giao: {task.receiver_address}</p>
