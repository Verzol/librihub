...
export default function MyDeliveriesPage() {
  ...
  return (
    <>
      <PageHeader
        hero
        heroIcon={<PackageCheck className="h-3.5 w-3.5" />}
        title="Nhiệm vụ của tôi"
        description="Cập nhật đã lấy, đã giao hoặc thất bại cho nhiệm vụ được gán."
        heroStat={
          <>
            <p className="text-sm font-medium text-blue-200">Vận đơn</p>
            <div className="mt-1 text-4xl font-bold text-white">{deliveries.length}</div>
            <p className="mt-1 text-sm text-blue-200">cần xử lý</p>
          </>
        }
      />
            ...
            <div className="flex items-start justify-between gap-3 max-md:flex-col">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="text-base font-semibold text-slate-900">Vận đơn #{delivery.delivery_id}</h2>
                  <Badge value={delivery.delivery_status} />
                </div>
                <p className="mt-2 text-base text-slate-500">Giao dịch #{delivery.transaction_id}</p>
                <p className="text-base text-slate-500">Lấy: {delivery.pickup_address ?? "Chưa có điểm lấy"}</p>
                <p className="text-base text-slate-500">Nhận: {delivery.receiver_address}</p>
                <p className="text-base text-slate-500">Dự kiến: {formatDate(delivery.expected_delivery_at)}</p>
              </div>
