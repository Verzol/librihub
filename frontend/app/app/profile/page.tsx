...
import { authApi, reviewsApi, transactionsApi } from "@/lib/api";
import { errorMessage } from "@/lib/api/client";
import type { Review, Transaction } from "@/lib/api/types";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([reviewsApi.forUser(token, user.user_id), transactionsApi.list(token)])
      .then(([reviewData, transactionData]) => {
      ...
      <div className="grid grid-cols-[360px_minmax(0,1fr)] gap-5 max-lg:grid-cols-1">
        <div className="flex flex-col gap-5">
          <Card className="overflow-hidden p-0">
            <div className="bg-gradient-to-br from-blue-700 to-blue-500 px-5 pb-16 pt-6 text-white">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-blue-100">LibriHub Profile</p>
                  <h2 className="mt-2 text-2xl font-bold">{user.full_name}</h2>
                </div>
                <Badge value={user.role} />
              </div>
            </div>
            <div className="-mt-12 px-5 pb-5">
              <div className="flex h-24 w-24 items-center justify-center rounded-[2rem] border-4 border-white bg-blue-100 text-4xl font-bold text-blue-700 shadow-lg">
              ...
              <div className="mt-5 grid grid-cols-2 gap-3">
                <Metric icon={Award} label="LibriPoint" value={String(user.current_points)} />
                <Metric icon={Star} label="Đánh giá" value={averageRating ? averageRating.toFixed(1) : "-"} />
                <Metric icon={ShieldCheck} label="Hoàn tất" value={String(completedCount)} />
                <Metric icon={CalendarDays} label="Tham gia" value={formatDate(user.created_at).split(" ")[0]} />
              </div>
            </div>
          </Card>

          <Card>
            <h2 className="text-sm font-bold text-slate-950">Thông tin hiển thị</h2>
        ...
        <div className="flex min-w-0 flex-col gap-5">
          <Card>
            <div className="mb-5 flex items-start justify-between gap-4 max-sm:flex-col">
              <div>
                <h2 className="text-lg font-bold text-slate-950">Chỉnh sửa hồ sơ</h2>
          ...
          <Card>
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Uy tín từ các giao dịch đã hoàn tất.</p>
              </div>
            ...
            <div className="mt-4 grid gap-3">
              ...
              {reviews.slice(0, 5).map((review) => (
                <div key={review.review_id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
